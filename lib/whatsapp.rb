class WhatsApp
	def self.send_message phone_number, message
		HTTParty.post("https://ongair.im/api/v1/base/send", body: {token: Rails.application.secrets.ongair_token, phone_number: phone_number, text: message, thread: true})
	end

	def self.send_image phone_number, image_url
		token = Rails.application.secrets.ongair_token
		url = "https://ongair.im/api/v1/base/send_image?token=#{token}"
